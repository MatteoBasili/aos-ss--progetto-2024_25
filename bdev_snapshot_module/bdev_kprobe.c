 ...
 * Si aggancia a "submit_bio" (o a "blk_mq_submit_bio" se preferito).
 *
 * NOTE:
 ...
 * Per altre arch potrebbe essere necessario adattare.
 */
...
static int is_write_bio(const struct bio *bio)
{
    ...
    case REQ_OP_DISCARD:   /* opzionale: conta come “write” logica */
        return 1;
    default:
        return 0;
    }
...
static int submit_bio_pre(struct kprobe *p, struct pt_regs *regs)
{
    ...
    /* Recupera info utili per F7 (qui solo log) */
    ...
    /*
     * In F7/F8 qui verrà creato e enqueue-ato un lavoro (work_struct)
     * con i metadati necessari per leggere il blocco originale e salvarlo.
     */
...
int bdev_kprobe_init(void)
{
    ...
    /* Prova a collegare a submit_bio; se vuoi puoi cambiare il simbolo. */
